...
export default function SideNavbar() {
  ...
  const menuItems = [
    { icon: <RepositoryIcon fill="white" />, label: "Repositories", link: "/home" },
    { icon: <CodeReviewIcon />, label: "AI Code Review", link: "/YetToCome" },
    { icon: <CloudIcon />, label: "Cloud Security", link: "/YetToCome" },
    { icon: <BookIcon />, label: "How to Use", link: "/YetToCome" },
    { icon: <SettingIcon />, label: "Settings", link: "/YetToCome" },
  ];

  const menuItemsMobile = [
    { icon: <RepositoryIcon fill="#414651" />, label: "Repositories", link: "/home" },
    { icon: <CodeReviewIcon />, label: "AI Code Review", link: "/YetToCome" },
    { icon: <CloudIcon />, label: "Cloud Security", link: "/YetToCome" },
    { icon: <BookIcon />, label: "How to Use", link: "/YetToCome" },
    { icon: <SettingIcon />, label: "Settings", link: "/YetToCome" },
    ...
    <>
      <div className="adf max-lg:hidden py-6 px-5 border-r border-r-[#E9EAEB] fixed left-0 top-0 w-[240px] font-inter flex flex-col justify-between bg-white h-screen">
        ...
        <div className="flex flex-col gap-2">
          <Link to="/YetToCome" className="inline-flex cursor-pointer px-3 py-2 gap-x-2 text-[#414651] font-semibold">
            <div>
              <PhoneIcon />
            </div>
            <div>Support</div>
      ...
      <div className={"adffsdf lg:hidden w-screen fixed top-0 left-0 z-40 bg-white p-4 border-b border-b-[#E9EAEB]"}>
        ...
        <div className={`transition-all cursor-pointer duration-300 overflow-hidden ${open ? "max-h-[500px]" : "max-h-0"}`}>
          ...
          <div className="flex flex-col gap-2 mt-4">
            ...
            <Link to="/YetToCome" className="inline-flex cursor-pointer gap-x-2 font-semibold px-[14px] py-2 leading-6 rounded-lg text-[#414651]">
              <div>
                <PhoneIcon />
              </div>
              <div>Support</div>
      ...
      <div
        className={`fixed inset-0 bg-black bg-opacity-50 z-30 ${open ? "opacity-100" : "opacity-0 pointer-events-none"} transition-opacity duration-300`}
